...
def SimplexTree(stbase, fct, dim, card): #dim must be array, possibility to consider several dimensions
    ...
    for s1, s2 in pairs:
        ...
        for d in dim :
            ...
            elif len(s1) == d+1 and len(s2)==0: #These features die at infinity, we assert that they die at max(f)
                l1 = np.array(s1)
                i1 = l1[np.argmax(fct[l1])]
                indices.append(i1)
                indices.append(val)
                #Compute lifetime
                pers.append(M-st.filtration(s1))
